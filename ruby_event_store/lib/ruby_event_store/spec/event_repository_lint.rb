RSpec.shared_examples :event_repository do |repository_class|
  ...
  explicit_version_interchanging_modes.each do |mode|
    specify ":#{mode} queries for last position in given stream" do
      repository.append_to_stream([
        eventA = TestDomainEvent.new(event_id: SecureRandom.uuid),
        eventB = TestDomainEvent.new(event_id: SecureRandom.uuid),
    ...
    specify ":#{mode} queries for last position and follows in incremental way" do
    ...
    specify ":#{mode} is compatible with manual expectation" do
      repository.append_to_stream([
        event0 = TestDomainEvent.new(event_id: SecureRandom.uuid),
        event1 = TestDomainEvent.new(event_id: SecureRandom.uuid),
      ], 'stream', mode)
    ...
    specify "manual is compatible with :#{mode} expectation" do
      repository.append_to_stream([
        event0 = TestDomainEvent.new(event_id: SecureRandom.uuid),
        event1 = TestDomainEvent.new(event_id: SecureRandom.uuid),
      ], 'stream', :none)
